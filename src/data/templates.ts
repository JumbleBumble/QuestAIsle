import { templateSchema, GameTemplate } from '../types/game'
import { listJsonRecords, writeJsonRecord, deleteRecord, nextSlug, readJsonRecord } from '../utils/fileStorage'
import { z } from 'zod'
...
export async function saveTemplate(draft: TemplateDraft, existing?: GameTemplate) {
...
export function validateTemplateDraft(draft: TemplateDraft) {
	return z
		.object({
			...
			valueDefinitions: z.array(z.any()).min(1, 'Add at least one tracked value'),
		})
		.parse(draft)
}
